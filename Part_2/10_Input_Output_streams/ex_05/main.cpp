...
ostream &operator<<(ostream &os, Year &y) {
	...
	for(const auto& m : y.month) {
		...
		os << "\tmonth: " << int_to_month(m.month) << endl;;

		for(size_t d = 0; d < m.day.size(); ++d) {
			for(size_t i = 0; i < m.day[d].hour.size(); ++i) {
				if(m.day[d].hour[i] == not_a_reading)
					continue;

				os << "\t\t" << i << " temp: " << m.day[d].hour[i] << endl;
			}
		}
